...
const submitHandlerCreate = async ({
  ...
  try {
    const { client, error, success } = await createType({ values });
...
const submitHandlerEdit = async ({
  ...
  try {
    const { client, error, success } = await updateType({
      id: selectedRow.id,
      values,
    });
    ...
    toast.error("Error al actualizar el tipo. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  try {
    ...
    console.error(error);
    toast.error("Error al eliminar los tipos. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
