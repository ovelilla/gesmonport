...
const Detail = ({ architrave }: DetailProps) => {
  const [selectedImage, setSelectedImage] = useState(
    architrave.images[0]?.url || "",
  );
      ...
      <Card className="max-w-auto p-0 text-sm">
        <CardContent>
          <div className="flex flex-col gap-1">
            ...
            {architrave.description && (
              <p className="text-muted-foreground">{architrave.description}</p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {architrave.reference ?? "-"}
              </dd>

              <dt className="font-medium">Familia</dt>
              <dd className="whitespace-pre-line">
                {architrave.family?.name ?? "-"}
              </dd>

              <dt className="font-medium">Tipo</dt>
              <dd className="whitespace-pre-line">
                {architrave.type?.name ?? "-"}
              </dd>

              <dt className="font-medium">Acabado</dt>
              <dd className="whitespace-pre-line">
                {architrave.finish?.name ?? "-"}
              </dd>
            </dl>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {format(new Date(architrave.createdAt), "PPP", { locale: es })}
              </dd>
              <dt className="font-semibold">Última actualización:</dt>
              <dd className="whitespace-pre-line">
                {format(new Date(architrave.updatedAt), "PPP", { locale: es })}
              </dd>
            </dl>
          </div>
        </CardContent>
