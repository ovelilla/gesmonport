...
// Hooks
import { ArchitraveFinishHook } from "./hooks/architrave-finish.hook";
// Types
import type { ArchitraveFinishProps } from "./types/architrave-finish.container.types";

const ArchitraveFinishContainer = ({
  architraveFinish,
}: ArchitraveFinishProps) => {
  const {
    data,
    handleAddColumn,
    handleAddRow,
    ...
    setData,
    theme,
  } = ArchitraveFinishHook({
    architraveFinish,
  });
  return (
    <div className="flex grow flex-col gap-2 overflow-hidden p-4">
      <h1 className="text-xl font-semibold">{architraveFinish.name}</h1>
      <Tabs defaultValue="details" className="flex-1 gap-4 overflow-hidden">
        ...
        <TabsContent className="overflow-auto" value="details">
          <Detail architraveFinish={architraveFinish} />
        </TabsContent>
...
export { ArchitraveFinishContainer };
