...
const Detail = ({ architraveFinish }: DetailProps) => {
  const [selectedImage, setSelectedImage] = useState(
    architraveFinish.images[0]?.url || "",
  );
      ...
      <div className="flex flex-col gap-4">
        ...
        <Carousel className="w-full">
          <CarouselContent className="-ml-4">
            {architraveFinish.images.map((image) => (
        ...
        <CardContent>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {format(new Date(architraveFinish.createdAt), "PPP", {
                  locale: es,
                })}
              </dd>
              <dt className="font-semibold">Última actualización:</dt>
              <dd className="whitespace-pre-line">
                {format(new Date(architraveFinish.updatedAt), "PPP", {
                  locale: es,
                })}
              </dd>
            </dl>
