...
// Actions
import { saveArchitraveFinishPrices } from "../actions/architrave-finish.actions";
// Types
import type {
...
} from "./types/architrave-finish.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveArchitraveFinishPrices({
      architraveFinishId: architraveFinish.id,
      prices: parsedPrices,
    });
...
const ArchitraveFinishHandlers = ({
  data,
  architraveFinish,
  setData,
  setLoading,
  setOriginalData,
}: ArchitraveFinishHandlersProps): ArchitraveFinishHandlersReturn => {
  return {
...
export { ArchitraveFinishHandlers };
