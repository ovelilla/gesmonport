...
const createFamily = async ({
  ...
  try {
    ...
    try {
      ...
      const transformed = {
        ...newFamily,
        models: newFamily.models.map((model) => model.architraveModel),
      };
...
const deleteMultipleFamilies = async ({
  ...
  try {
    const images = await prisma.architraveFamilyImage.findMany({
      where: { architraveFamilyId: { in: ids } },
      select: { publicId: true },
    });
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.architraveFamilyImage.findMany({
        where: { architraveFamilyId: id, url: { in: toDelete } },
        select: { publicId: true },
      });
    ...
    try {
      ...
      const transformed = {
        ...updatedFamily,
        models: updatedFamily.models.map((model) => model.architraveModel),
      };
