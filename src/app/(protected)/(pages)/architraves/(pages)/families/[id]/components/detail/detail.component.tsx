...
const Detail = ({ architraveFamily }: DetailProps) => {
  const [selectedImage, setSelectedImage] = useState(
    architraveFamily.images[0]?.url || "",
  );
      ...
      <div className="flex flex-col gap-4">
        ...
        <Carousel className="w-full">
          <CarouselContent className="-ml-4">
            {architraveFamily.images.map((image) => (
      ...
      <Card className="max-w-auto p-0 text-sm">
        <CardContent>
          <div className="flex flex-col gap-1">
            <h1 className="text-xl font-semibold">{architraveFamily.name}</h1>
            {architraveFamily.description && (
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {architraveFamily.reference}
              </dd>
              <dt className="font-medium">Modelos</dt>
              <dd className="whitespace-pre-line">
                {architraveFamily.models && architraveFamily.models.length > 0
                  ? architraveFamily.models.map((m) => m.name).join(", ")
                  : "-"}
              </dd>
            </dl>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {format(new Date(architraveFamily.createdAt), "PPP", {
                  locale: es,
                })}
              </dd>
              <dt className="font-semibold">Última actualización:</dt>
              <dd className="whitespace-pre-line">
                {format(new Date(architraveFamily.updatedAt), "PPP", {
                  locale: es,
                })}
              </dd>
            </dl>
