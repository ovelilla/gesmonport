...
import type {
  ...
  SubmitHandlerProps,
} from "./types/families.handlers.types";
...
const navigateHandler = ({ row, router }: NavigateHandlerProps): void => {
  router.push(`/architraves/options/families/${row.id}`);
};
...
const submitHandlerCreate = async ({
  ...
  try {
    const { family, error, success } = await createFamily({
      newImages,
      values,
    });
    ...
    console.error(error);
    toast.error("Error al crear la familia. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { family, error, success } = await updateFamily({
      id: selectedRow.id,
      newImages,
      toDelete,
      values,
    ...
    if (success && family) {
      setData((prev) =>
        prev.map((item) => (item.id === family.id ? family : item)),
      );
      form.reset();
      setOpenDialog(false);
      setSelectedRow(null);
    ...
    console.error(error);
    toast.error(
      "Error al actualizar la familia. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteFamily({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error("Error al eliminar la familia. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleFamilies({
    ...
    console.error(error);
    toast.error(
      "Error al eliminar las familias. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const FamiliesHandlers = ({
  ...
  setToDelete,
  toDelete,
}: FamiliesHandlersProps): FamiliesHandlersReturn => {
  return {
    handleCreate: () =>
      createHandler({
        setExistingImages,
