...
// Actions
import { saveArchitraveModelPrices } from "../actions/architrave-model.actions";
// Types
import type {
...
} from "./types/architrave-model.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveArchitraveModelPrices({
      architraveModelId: architraveModel.id,
      prices: parsedPrices,
    });
...
const ArchitraveModelHandlers = ({
  data,
  architraveModel,
  setData,
  setLoading,
  setOriginalData,
}: ArchitraveModelHandlersProps): ArchitraveModelHandlersReturn => {
  return {
...
export { ArchitraveModelHandlers };
