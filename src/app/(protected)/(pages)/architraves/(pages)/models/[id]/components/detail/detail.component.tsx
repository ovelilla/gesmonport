...
const Detail = ({ architraveModel }: DetailProps) => {
  const [selectedImage, setSelectedImage] = useState(
    architraveModel.images[0]?.url || "",
  );
      ...
      <div className="flex flex-col gap-4">
        ...
        <Carousel className="w-full">
          <CarouselContent className="-ml-4">
            {architraveModel.images.map((image) => (
      ...
      <Card className="max-w-auto p-0 text-sm">
        <CardContent>
          <div className="flex flex-col gap-1">
            <h1 className="text-xl font-semibold">{architraveModel.name}</h1>
            {architraveModel.description && (
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {architraveModel.reference}
              </dd>
              <dt className="font-medium">Acabados</dt>
              <dd className="whitespace-pre-line">
                {architraveModel.finishes && architraveModel.finishes.length > 0
                  ? architraveModel.finishes.map((f) => f.name).join(", ")
                  : "-"}
              </dd>
            </dl>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {format(new Date(architraveModel.createdAt), "PPP", {
                  locale: es,
                })}
              </dd>
              <dt className="font-semibold">Última actualización:</dt>
              <dd className="whitespace-pre-line">
                {format(new Date(architraveModel.updatedAt), "PPP", {
                  locale: es,
                })}
              </dd>
            </dl>
