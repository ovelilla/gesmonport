...
} from "./types/models.hook.utils.types";

function getColumnsConfig({
  handleDelete,
  handleEdit,
  handleNavigate,
}: GetColumnsConfigProps<Model>): GetColumnsConfigReturn<Model> {
  return [
    {
    ...
    {
      cell: ({ row }) => (
        <Link href={`/architraves/models/${row.original.id}`} prefetch={false}>
    ...
    },
    {
      cell: ({ row }) => (
        <RowActions
          ...
          actions={[
            { icon: Eye, label: "Ver modelo", onClick: handleNavigate },
