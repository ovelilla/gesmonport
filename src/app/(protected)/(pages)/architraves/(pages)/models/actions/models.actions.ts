...
const createModel = async ({
  ...
  try {
    ...
    try {
      const newModel = await prisma.architraveModel.create({
        data: {
          ...validatedFields.data,
          finishes: {
            create: validatedFields.data.finishes.map((finishId) => ({
              architraveFinishId: finishId,
            })),
          },
          images: {
            create: validImages,
      ...
      const transformed = {
        ...newModel,
        finishes: newModel.finishes.map((finish) => finish.architraveFinish),
      };
...
const deleteModel = async ({
  ...
  try {
    const images = await prisma.architraveModelImage.findMany({
      where: { architraveModelId: id },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.architraveModel.delete({ where: { id } });
...
const deleteMultipleModels = async ({
  ...
  try {
    const images = await prisma.architraveModelImage.findMany({
      where: { architraveModelId: { in: ids } },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.architraveModel.deleteMany({ where: { id: { in: ids } } });
...
const readModels = async (): Promise<ReadModelsReturn> => {
  try {
    const models = await prisma.architraveModel.findMany({
      orderBy: { name: "asc" },
      include: {
        finishes: { include: { architraveFinish: true } },
        images: true,
...
const updateModel = async ({
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.architraveModelImage.findMany({
        where: { architraveModelId: id, url: { in: toDelete } },
        select: { publicId: true },
      });
      ...
      await prisma.architraveModelImage.deleteMany({
        where: { architraveModelId: id, url: { in: toDelete } },
      });
    }
    ...
    try {
      const updatedModel = await prisma.architraveModel.update({
        where: { id },
        data: {
          ...validatedFields.data,
          finishes: {
            deleteMany: {},
            create: validatedFields.data.finishes.map((finishId) => ({
              architraveFinish: { connect: { id: finishId } },
            })),
          },
      ...
      const transformed = {
        ...updatedModel,
        finishes: updatedModel.finishes.map(
