...
import type {
  ...
  SubmitHandlerProps,
} from "./types/models.handlers.types";
...
const editHandler = ({
  ...
  const transformedRow = {
    ...row,
    finishes: row.finishes.map((finish) => finish.id),
    images: [],
  };
...
const navigateHandler = ({ row, router }: NavigateHandlerProps): void => {
  router.push(`/architraves/models/${row.id}`);
};
...
const submitHandlerCreate = async ({
  ...
  try {
    const { model, error, success } = await createModel({
      newImages,
      values,
    });
    ...
    console.error(error);
    toast.error("Error al crear el modelo. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { model, error, success } = await updateModel({
      id: selectedRow.id,
      newImages,
      toDelete,
      values,
    ...
    console.error(error);
    toast.error("Error al actualizar el modelo. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteModel({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error("Error al eliminar el modelo. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleModels({
    ...
    console.error(error);
    toast.error("Error al eliminar los modelos. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const ModelsHandlers = ({
  ...
  setToDelete,
  toDelete,
}: ModelsHandlersProps): ModelsHandlersReturn => {
  return {
    handleCreate: () =>
      createHandler({
        setExistingImages,
