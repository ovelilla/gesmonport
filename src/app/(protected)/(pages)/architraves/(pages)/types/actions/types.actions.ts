...
const createType = async ({
  ...
  try {
    ...
    try {
      const newType = await prisma.architraveType.create({
        data: {
          ...validatedFields.data,
          families: {
            create: validatedFields.data.families.map((familyId) => ({
              architraveFamilyId: familyId,
            })),
          },
          images: {
            create: validImages,
      ...
      const transformed = {
        ...newType,
        families: newType.families.map((family) => family.architraveFamily),
      };
...
const deleteType = async ({
  ...
  try {
    const images = await prisma.architraveTypeImage.findMany({
      where: { architraveTypeId: id },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.architraveType.delete({ where: { id } });
...
const deleteMultipleTypes = async ({
  ...
  try {
    const images = await prisma.architraveTypeImage.findMany({
      where: { architraveTypeId: { in: ids } },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.architraveType.deleteMany({ where: { id: { in: ids } } });
...
const updateType = async ({
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.architraveTypeImage.findMany({
        where: { architraveTypeId: id, url: { in: toDelete } },
        select: { publicId: true },
      });
      ...
      await prisma.architraveTypeImage.deleteMany({
        where: { architraveTypeId: id, url: { in: toDelete } },
      });
    }
    ...
    try {
      const updatedType = await prisma.architraveType.update({
        where: { id },
        data: {
          ...validatedFields.data,
          families: {
      ...
      const transformed = {
        ...updatedType,
        families: updatedType.families.map((family) => family.architraveFamily),
      };
