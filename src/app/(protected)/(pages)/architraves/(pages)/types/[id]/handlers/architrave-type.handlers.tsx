...
// Actions
import { saveArchitraveTypePrices } from "../actions/architrave-type.actions";
// Types
import type {
...
} from "./types/architrave-type.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveArchitraveTypePrices({
      architraveTypeId: architraveType.id,
      prices: parsedPrices,
    });
...
const ArchitraveTypeHandlers = ({
  data,
  architraveType,
  setData,
  setLoading,
  setOriginalData,
}: ArchitraveTypeHandlersProps): ArchitraveTypeHandlersReturn => {
  return {
