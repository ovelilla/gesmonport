...
const Detail = ({ architraveType }: DetailProps) => {
  const [selectedImage, setSelectedImage] = useState(
    architraveType.images[0]?.url || "",
  );
      ...
      <div className="flex flex-col gap-4">
        ...
        <Carousel className="w-full">
          <CarouselContent className="-ml-4">
            {architraveType.images.map((image) => (
      ...
      <Card className="max-w-auto p-0 text-sm">
        <CardContent>
          <div className="flex flex-col gap-1">
            <h1 className="text-xl font-semibold">{architraveType.name}</h1>
            {architraveType.description && (
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {architraveType.reference}
              </dd>
              <dt className="font-medium">Familias</dt>
              <dd className="whitespace-pre-line">
                {architraveType.families && architraveType.families.length > 0
                  ? architraveType.families.map((f) => f.name).join(", ")
                  : "-"}
              </dd>
            </dl>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {format(new Date(architraveType.createdAt), "PPP", {
                  locale: es,
                })}
              </dd>
              <dt className="font-semibold">Última actualización:</dt>
              <dd className="whitespace-pre-line">
                {format(new Date(architraveType.updatedAt), "PPP", {
                  locale: es,
                })}
              </dd>
            </dl>
