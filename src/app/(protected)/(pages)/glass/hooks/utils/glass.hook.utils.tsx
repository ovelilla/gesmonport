...
} from "./types/glass.hook.utils.types";

function getColumnsConfig({
  handleDelete,
  handleEdit,
  handleNavigate,
}: GetColumnsConfigProps<Glass>): GetColumnsConfigReturn<Glass> {
  return [
    {
    ...
    {
      cell: ({ row }) => (
        <Link href={`/glass/${row.original.id}`} prefetch={false}>
    ...
    {
      accessorKey: "model",
      header: ({ column }) => <ColumnSorter column={column} label="Modelo" />,
    ...
    },
    {
      cell: ({ row }) => (
        <RowActions
          ...
          actions={[
            { icon: Eye, label: "Ver vidrio", onClick: handleNavigate },
