...
const createFinish = async ({
  ...
  try {
    ...
    try {
      const newFinish = await prisma.glassFinish.create({
        data: {
...
const deleteFinish = async ({
  ...
  try {
    const images = await prisma.glassFinishImage.findMany({
      where: { glassFinishId: id },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.glassFinish.delete({ where: { id } });
...
const deleteMultipleFinishes = async ({
  ...
  try {
    const images = await prisma.glassFinishImage.findMany({
      where: { glassFinishId: { in: ids } },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.glassFinish.deleteMany({ where: { id: { in: ids } } });
...
const updateFinish = async ({
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.glassFinishImage.findMany({
        where: { glassFinishId: id, url: { in: toDelete } },
        select: { publicId: true },
      });
      ...
      await prisma.glassFinishImage.deleteMany({
        where: { glassFinishId: id, url: { in: toDelete } },
      });
    }
    ...
    try {
      const updatedFinish = await prisma.glassFinish.update({
        where: { id },
        data: {
          ...validatedFields.data,
          images: {
