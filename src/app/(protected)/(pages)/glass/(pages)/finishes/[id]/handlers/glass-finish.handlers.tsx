...
// Actions
import { saveGlassFinishPrices } from "../actions/glass-finish.actions";
// Types
import type {
...
} from "./types/glass-finish.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveGlassFinishPrices({
      glassFinishId: glassFinish.id,
      prices: parsedPrices,
    });
...
const GlassFinishHandlers = ({
  data,
  glassFinish,
  setData,
  setLoading,
  setOriginalData,
}: GlassFinishHandlersProps): GlassFinishHandlersReturn => {
  return {
