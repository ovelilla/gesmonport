...
// Actions
import { saveGlassModelPrices } from "../actions/glass-model.actions";
// Types
import type {
...
} from "./types/glass-model.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveGlassModelPrices({
      glassModelId: glassModel.id,
      prices: parsedPrices,
    });
...
const GlassModelHandlers = ({
  data,
  glassModel,
  setData,
  setLoading,
  setOriginalData,
}: GlassModelHandlersProps): GlassModelHandlersReturn => {
  return {
