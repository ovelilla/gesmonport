...
const Detail = ({ glassModel }: DetailProps) => {
  const [selectedImage, setSelectedImage] = useState(
    glassModel.images[0]?.url || "",
  );
      ...
      <Card className="max-w-auto p-0 text-sm">
        <CardContent>
          <div className="flex flex-col gap-1">
            <h1 className="text-xl font-semibold">{glassModel.name}</h1>
            {glassModel.description && (
              <p className="text-muted-foreground">{glassModel.description}</p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">{glassModel.reference}</dd>
              <dt className="font-medium">Acabados</dt>
              <dd className="whitespace-pre-line">
                {glassModel.finishes && glassModel.finishes.length > 0
                  ? glassModel.finishes.map((f) => f.name).join(", ")
                  : "-"}
              </dd>
            </dl>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {format(new Date(glassModel.createdAt), "PPP", { locale: es })}
              </dd>
              <dt className="font-semibold">Última actualización:</dt>
              <dd className="whitespace-pre-line">
                {format(new Date(glassModel.updatedAt), "PPP", { locale: es })}
              </dd>
            </dl>
          </div>
        </CardContent>
