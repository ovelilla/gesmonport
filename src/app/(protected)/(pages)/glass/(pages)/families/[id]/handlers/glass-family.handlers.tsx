...
// Actions
import { saveGlassFamilyPrices } from "../actions/glass-family.actions";
// Types
import type {
...
} from "./types/glass-family.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveGlassFamilyPrices({
      glassFamilyId: glassFamily.id,
      prices: parsedPrices,
    });
...
const GlassFamilyHandlers = ({
  data,
  glassFamily,
  setData,
  setLoading,
  setOriginalData,
}: GlassFamilyHandlersProps): GlassFamilyHandlersReturn => {
  return {
