...
const Detail = ({ glassFamily }: DetailProps) => {
  const [selectedImage, setSelectedImage] = useState(
    glassFamily.images[0]?.url || "",
  );
      ...
      <Card className="max-w-auto p-0 text-sm">
        <CardContent>
          <div className="flex flex-col gap-1">
            <h1 className="text-xl font-semibold">{glassFamily.name}</h1>
            {glassFamily.description && (
              <p className="text-muted-foreground">{glassFamily.description}</p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">{glassFamily.reference}</dd>
              <dt className="font-medium">Modelos</dt>
              <dd className="whitespace-pre-line">
                {glassFamily.models && glassFamily.models.length > 0
                  ? glassFamily.models.map((m) => m.name).join(", ")
                  : "-"}
              </dd>
            </dl>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {format(new Date(glassFamily.createdAt), "PPP", { locale: es })}
              </dd>
              <dt className="font-semibold">Última actualización:</dt>
              <dd className="whitespace-pre-line">
                {format(new Date(glassFamily.updatedAt), "PPP", { locale: es })}
              </dd>
            </dl>
          </div>
        </CardContent>
