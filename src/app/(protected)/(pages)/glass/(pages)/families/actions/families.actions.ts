...
const createFamily = async ({
  ...
  try {
    ...
    try {
      ...
      const transformed = {
        ...newFamily,
        models: newFamily.models.map((model) => model.glassModel),
      };
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.glassFamilyImage.findMany({
        where: { glassFamilyId: id, url: { in: toDelete } },
        select: { publicId: true },
      });
    ...
    try {
      ...
      const transformed = {
        ...updatedFamily,
        models: updatedFamily.models.map((model) => model.glassModel),
      };
