...
const Detail = ({ glassType }: DetailProps) => {
  const [selectedImage, setSelectedImage] = useState(
    glassType.images[0]?.url || "",
  );
      ...
      <Card className="max-w-auto p-0 text-sm">
        <CardContent>
          <div className="flex flex-col gap-1">
            <h1 className="text-xl font-semibold">{glassType.name}</h1>
            {glassType.description && (
              <p className="text-muted-foreground">{glassType.description}</p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">{glassType.reference}</dd>
              <dt className="font-medium">Familias</dt>
              <dd className="whitespace-pre-line">
                {glassType.families && glassType.families.length > 0
                  ? glassType.families.map((f) => f.name).join(", ")
                  : "-"}
              </dd>
            </dl>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {format(new Date(glassType.createdAt), "PPP", { locale: es })}
              </dd>
              <dt className="font-semibold">Última actualización:</dt>
              <dd className="whitespace-pre-line">
                {format(new Date(glassType.updatedAt), "PPP", { locale: es })}
              </dd>
            </dl>
          </div>
        </CardContent>
