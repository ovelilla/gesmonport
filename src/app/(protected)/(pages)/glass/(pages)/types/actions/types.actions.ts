...
const createType = async ({
  ...
  try {
    ...
    try {
      const newType = await prisma.glassType.create({
        data: {
          ...validatedFields.data,
          families: {
            create: validatedFields.data.families.map((familyId) => ({
              glassFamilyId: familyId,
            })),
          },
          images: {
            create: validImages,
      ...
      const transformed = {
        ...newType,
        families: newType.families.map((family) => family.glassFamily),
      };
...
const deleteType = async ({
  ...
  try {
    const images = await prisma.glassTypeImage.findMany({
...
const deleteMultipleTypes = async ({
  ...
  try {
    const images = await prisma.glassTypeImage.findMany({
      where: { glassTypeId: { in: ids } },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.glassType.deleteMany({ where: { id: { in: ids } } });
...
const updateType = async ({
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.glassTypeImage.findMany({
        where: { glassTypeId: id, url: { in: toDelete } },
        select: { publicId: true },
      });
      ...
      await prisma.glassTypeImage.deleteMany({
        where: { glassTypeId: id, url: { in: toDelete } },
      });
    }
    ...
    try {
      ...
      const transformed = {
        ...updatedType,
        families: updatedType.families.map((family) => family.glassFamily),
      };
