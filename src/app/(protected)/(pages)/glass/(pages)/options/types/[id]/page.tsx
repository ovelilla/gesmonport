...
const generateMetadata = async ({
  ...
  if (!type) {
    return {
      title: "Tipo de vidrio no encontrado",
      description: "Tipo de vidrio no encontrado.",
    };
  }

  return {
    title: `Tipo de vidrio ${type.name}`,
    description: `Tipo de vidrio ${type.name}.`,
  };
};
...
const TypePage = async ({ params }: TypePageProps) => {
  ...
  if (!type) {
    return <div className="p-4">Tipo de vidrio no encontrado.</div>;
  }
