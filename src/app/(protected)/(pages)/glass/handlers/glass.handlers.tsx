...
const submitHandlerCreate = async ({
  ...
  try {
    const { glass, error, success } = await createGlass({
      newImages,
      values,
    });
    ...
    console.error(error);
    toast.error("Error al crear el vidrio. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { glass, error, success } = await updateGlass({
      id: selectedRow.id,
      newImages,
      toDelete,
      values,
    ...
    if (success && glass) {
      setData((prev) =>
        prev.map((item) => (item.id === glass.id ? glass : item)),
      );
      form.reset();
      setOpenDialog(false);
      setSelectedRow(null);
    ...
    console.error(error);
    toast.error(
      "Error al actualizar el vidrio. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteGlass({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error("Error al eliminar el vidrio. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleGlass({
    ...
    console.error(error);
    toast.error(
      "Error al eliminar los vidrios. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
