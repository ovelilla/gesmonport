...
const createFamily = async ({
  ...
  try {
    ...
    try {
      const newFamily = await prisma.frameFamily.create({
        data: {
      ...
      const transformed = {
        ...newFamily,
        models: newFamily.models.map((model) => model.frameModel),
      };
...
const deleteFamily = async ({
  ...
  try {
    const images = await prisma.frameFamilyImage.findMany({
      where: { frameFamilyId: id },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.frameFamily.delete({ where: { id } });
...
const deleteMultipleFamilies = async ({
  ...
  try {
    const images = await prisma.frameFamilyImage.findMany({
      where: { frameFamilyId: { in: ids } },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.frameFamily.deleteMany({ where: { id: { in: ids } } });
...
const readModels = async (): Promise<ReadModelsReturn> => {
  try {
    const models = await prisma.frameModel.findMany({
      orderBy: { name: "asc" },
    });
    return models;
  } catch (error) {
...
const updateFamily = async ({
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.frameFamilyImage.findMany({
        where: { frameFamilyId: id, url: { in: toDelete } },
        select: { publicId: true },
      });
      ...
      await prisma.frameFamilyImage.deleteMany({
        where: { frameFamilyId: id, url: { in: toDelete } },
      });
    }
    ...
    try {
      const updatedFamily = await prisma.frameFamily.update({
        where: { id },
        data: {
          ...validatedFields.data,
          images: {
      ...
      const transformed = {
        ...updatedFamily,
        models: updatedFamily.models.map((model) => model.frameModel),
      };
