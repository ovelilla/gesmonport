...
} from "./types/families.hook.utils.types";

function getColumnsConfig({
  handleDelete,
  handleEdit,
  handleNavigate,
}: GetColumnsConfigProps<Family>): GetColumnsConfigReturn<Family> {
  return [
    {
    ...
    {
      cell: ({ row }) => (
        <Link href={`/frames/families/${row.original.id}`} prefetch={false}>
    ...
    },
    {
      cell: ({ row }) => (
        <RowActions
          ...
          actions={[
            { icon: Eye, label: "Ver familia", onClick: handleNavigate },
