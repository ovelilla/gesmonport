...
const createFinish = async ({
  ...
  try {
    ...
    try {
      const newFinish = await prisma.frameFinish.create({
        data: {
...
const deleteFinish = async ({
  ...
  try {
    const images = await prisma.frameFinishImage.findMany({
      where: { frameFinishId: id },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.frameFinish.delete({ where: { id } });
...
const deleteMultipleFinishes = async ({
  ...
  try {
    const images = await prisma.frameFinishImage.findMany({
      where: { frameFinishId: { in: ids } },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.frameFinish.deleteMany({ where: { id: { in: ids } } });
...
const updateFinish = async ({
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.frameFinishImage.findMany({
        where: { frameFinishId: id, url: { in: toDelete } },
        select: { publicId: true },
      });
      ...
      await prisma.frameFinishImage.deleteMany({
        where: { frameFinishId: id, url: { in: toDelete } },
      });
    }
    ...
    try {
      const updatedFinish = await prisma.frameFinish.update({
        where: { id },
        data: {
          ...validatedFields.data,
          images: {
