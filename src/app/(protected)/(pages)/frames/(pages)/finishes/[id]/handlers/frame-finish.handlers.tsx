...
// Actions
import { saveFrameFinishPrices } from "../actions/frame-finish.actions";
// Types
import type {
...
} from "./types/frame-finish.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveFrameFinishPrices({
      frameFinishId: frameFinish.id,
      prices: parsedPrices,
    });
...
const FrameFinishHandlers = ({
  data,
  frameFinish,
  setData,
  setLoading,
  setOriginalData,
}: FrameFinishHandlersProps): FrameFinishHandlersReturn => {
  return {
