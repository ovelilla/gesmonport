...
const Detail = ({ frameModel }: DetailProps) => {
  const [selectedImage, setSelectedImage] = useState(
    frameModel.images[0]?.url || "",
  );
      ...
      <Card className="max-w-auto p-0 text-sm">
        <CardContent>
          <div className="flex flex-col gap-1">
            <h1 className="text-xl font-semibold">{frameModel.name}</h1>
            {frameModel.description && (
              <p className="text-muted-foreground">{frameModel.description}</p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">{frameModel.reference}</dd>
              <dt className="font-medium">Acabados</dt>
              <dd className="whitespace-pre-line">
                {frameModel.finishes && frameModel.finishes.length > 0
                  ? frameModel.finishes.map((f) => f.name).join(", ")
                  : "-"}
              </dd>
            </dl>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {format(new Date(frameModel.createdAt), "PPP", { locale: es })}
              </dd>
              <dt className="font-semibold">Última actualización:</dt>
              <dd className="whitespace-pre-line">
                {format(new Date(frameModel.updatedAt), "PPP", { locale: es })}
              </dd>
            </dl>
          </div>
        </CardContent>
