...
// Actions
import { saveFrameModelPrices } from "../actions/frame-model.actions";
// Types
import type {
...
} from "./types/frame-model.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveFrameModelPrices({
      frameModelId: frameModel.id,
      prices: parsedPrices,
    });
...
const FrameModelHandlers = ({
  data,
  frameModel,
  setData,
  setLoading,
  setOriginalData,
}: FrameModelHandlersProps): FrameModelHandlersReturn => {
  return {
