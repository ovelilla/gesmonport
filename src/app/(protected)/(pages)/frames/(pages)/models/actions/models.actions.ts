...
const createModel = async ({
  ...
  try {
    ...
    try {
      const newModel = await prisma.frameModel.create({
        data: {
      ...
      const transformed = {
        ...newModel,
        finishes: newModel.finishes.map((finish) => finish.frameFinish),
      };
...
const deleteModel = async ({
  ...
  try {
    const images = await prisma.frameModelImage.findMany({
      where: { frameModelId: id },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.frameModel.delete({ where: { id } });
...
const deleteMultipleModels = async ({
  ...
  try {
    const images = await prisma.frameModelImage.findMany({
      where: { frameModelId: { in: ids } },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.frameModel.deleteMany({ where: { id: { in: ids } } });
...
const updateModel = async ({
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.frameModelImage.findMany({
        where: { frameModelId: id, url: { in: toDelete } },
        select: { publicId: true },
      });
      ...
      await prisma.frameModelImage.deleteMany({
        where: { frameModelId: id, url: { in: toDelete } },
      });
    }
    ...
    try {
      const updatedModel = await prisma.frameModel.update({
        where: { id },
        data: {
          ...validatedFields.data,
          finishes: {
            deleteMany: {},
            create: validatedFields.data.finishes.map((finishId) => ({
              frameFinish: { connect: { id: finishId } },
            })),
          },
      ...
      const transformed = {
        ...updatedModel,
        finishes: updatedModel.finishes.map((finish) => finish.frameFinish),
      };
