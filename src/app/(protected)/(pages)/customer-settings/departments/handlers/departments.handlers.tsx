...
import type {
  ...
  SubmitHandlerProps,
} from "./types/departments.handlers.types";
...
const submitHandlerCreate = async ({
  ...
  try {
    const { client, error, success } = await createDepartment({ values });
    ...
    toast.error(
      "Error al crear el departamento. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerEdit = async ({
  ...
  try {
    const { client, error, success } = await updateDepartment({
      id: selectedRow.id,
      values,
    });
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteDepartment({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error(
      "Error al eliminar el departamento. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleDepartments({
...
const DepartmentsHandlers = ({
  ...
  setSelectedRow,
  setSelectedRows,
}: DepartmentsHandlersProps): DepartmentsHandlersReturn => {
  return {
