...
import type {
  ...
  SubmitHandlerProps,
} from "./types/positions.handlers.types";
...
const submitHandlerCreate = async ({
  ...
  try {
    const { client, error, success } = await createPosition({ values });
    ...
    toast.error("Error al crear el cargo. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { client, error, success } = await updatePosition({
      id: selectedRow.id,
      values,
    });
    ...
    toast.error("Error al actualizar el cargo. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deletePosition({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error("Error al eliminar el cargo. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultiplePositions({
    ...
    console.error(error);
    toast.error("Error al eliminar los cargos. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const PositionsHandlers = ({
  ...
  setSelectedRow,
  setSelectedRows,
}: PositionsHandlersProps): PositionsHandlersReturn => {
  return {
