...
import type {
  ...
  SubmitHandlerProps,
} from "./types/payment-methods.handlers.types";
...
const submitHandlerCreate = async ({
  ...
  try {
    const { client, error, success } = await createPaymentMethod({ values });
    ...
    toast.error(
      "Error al crear la forma de pago. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerEdit = async ({
  ...
  try {
    const { client, error, success } = await updatePaymentMethod({
      id: selectedRow.id,
      values,
    });
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultiplePaymentMethods({
...
const PaymentMethodsHandlers = ({
  ...
  setSelectedRow,
  setSelectedRows,
}: PaymentMethodsHandlersProps): PaymentMethodsHandlersReturn => {
  return {
