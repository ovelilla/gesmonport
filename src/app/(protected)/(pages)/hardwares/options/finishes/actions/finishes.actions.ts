...
const createFinish = async ({
  ...
  try {
    ...
    try {
      const newFinish = await prisma.hardwareFinish.create({
        data: {
...
const deleteFinish = async ({
  ...
  try {
    const images = await prisma.hardwareFinishImage.findMany({
      where: { finishId: id },
    ...
    await prisma.hardwareFinish.delete({ where: { id } });
...
const deleteMultipleFinishes = async ({
  ...
  try {
    ...
    await prisma.hardwareFinish.deleteMany({ where: { id: { in: ids } } });
...
const updateFinish = async ({
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.hardwareFinishImage.findMany({
      ...
      await prisma.hardwareFinishImage.deleteMany({
        where: { finishId: id, url: { in: toDelete } },
      });
    }
    ...
    try {
      const updatedFinish = await prisma.hardwareFinish.update({
        where: { id },
        data: {
          ...validatedFields.data,
          images: {
