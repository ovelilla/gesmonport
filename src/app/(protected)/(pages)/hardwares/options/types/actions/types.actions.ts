...
const createType = async ({
  ...
  const existing = await prisma.hardwareType.findUnique({
    where: { slug },
  });
  ...
  try {
    ...
    try {
      const newType = await prisma.hardwareType.create({
        data: {
...
const deleteType = async ({
  ...
  try {
    const images = await prisma.hardwareTypeImage.findMany({
      where: { typeId: id },
...
const deleteMultipleTypes = async ({
  ...
  try {
    ...
    await prisma.hardwareType.deleteMany({ where: { id: { in: ids } } });
...
const updateType = async ({
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.hardwareTypeImage.findMany({
      ...
      await prisma.hardwareTypeImage.deleteMany({
        where: { typeId: id, url: { in: toDelete } },
      });
    }
    ...
    try {
      const updatedType = await prisma.hardwareType.update({
        where: { id },
        data: {
          ...validatedFields.data,
          slug,
