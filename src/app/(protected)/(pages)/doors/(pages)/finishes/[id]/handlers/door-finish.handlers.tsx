...
// Actions
import { saveDoorFinishPrices } from "../actions/door-finish.actions";
// Types
import type {
...
} from "./types/door-finish.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveDoorFinishPrices({
      doorFinishId: doorFinish.id,
      prices: parsedPrices,
    });
...
const DoorFinishHandlers = ({
  data,
  doorFinish,
  setData,
  setLoading,
  setOriginalData,
}: DoorFinishHandlersProps): DoorFinishHandlersReturn => {
  return {
