...
const createFinish = async ({
  ...
  try {
    ...
    try {
      const newFinish = await prisma.doorFinish.create({
        data: {
...
const deleteFinish = async ({
  ...
  try {
    const images = await prisma.doorFinishImage.findMany({
      where: { doorFinishId: id },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.doorFinish.delete({ where: { id } });
...
const deleteMultipleFinishes = async ({
  ...
  try {
    const images = await prisma.doorFinishImage.findMany({
      where: { doorFinishId: { in: ids } },
      select: { publicId: true },
    });

    await Promise.all(images.map((img) => deleteImage(img.publicId)));

    await prisma.doorFinish.deleteMany({ where: { id: { in: ids } } });
...
const updateFinish = async ({
  ...
  try {
    if (toDelete.length > 0) {
      const imagesToDelete = await prisma.doorFinishImage.findMany({
        where: { doorFinishId: id, url: { in: toDelete } },
        select: { publicId: true },
      });
      ...
      await prisma.doorFinishImage.deleteMany({
        where: { doorFinishId: id, url: { in: toDelete } },
      });
    }
    ...
    try {
      const updatedFinish = await prisma.doorFinish.update({
        where: { id },
        data: {
          ...validatedFields.data,
          images: {
