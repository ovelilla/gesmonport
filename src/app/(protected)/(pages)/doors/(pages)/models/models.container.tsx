...
import { ModelForm } from "./components/form/form.component";
import { DataTable } from "@/components/data-table/data-table.component";
import { DialogWrapper } from "@/components/dialog-wrapper/dialog-wrapper.component";
// Hooks
import { ModelsHook } from "./hooks/models.hook";
// Types
...
const ModelsContainer = ({ models }: ModelsProps) => {
  ...
  return (
    <div className="flex grow flex-col gap-2 overflow-hidden p-4">
      <h1 className="text-2xl font-medium">Modelos de puertas</h1>
      <DataTable
        columns={columns}
        data={data}
      ...
      />
      <DialogWrapper
        description={`Rellena los campos para ${selectedRow ? "editar" : "crear"} un modelo.`}
        onOpenChange={handleOpenChangeDialog}
        open={openDialog}
        title={`${selectedRow ? "Editar" : "Crear"} modelo`}
      >
        <ModelForm
      ...
      <AlertDialogWrapper
        ...
        title={`Eliminar ${selectedRows.length > 1 ? "modelos" : "modelo"}`}
      />
    </div>
  );
};
