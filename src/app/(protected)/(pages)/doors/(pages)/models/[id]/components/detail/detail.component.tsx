...
const Detail = ({ doorModel }: DetailProps) => {
  const [selectedImage, setSelectedImage] = useState(
    doorModel.images[0]?.url || "",
  );
      ...
      <Card className="max-w-auto p-0 text-sm">
        <CardContent>
          <div className="flex flex-col gap-1">
            <h1 className="text-xl font-semibold">{doorModel.name}</h1>
            {doorModel.description && (
              <p className="text-muted-foreground">{doorModel.description}</p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">{doorModel.reference}</dd>
              <dt className="font-medium">Acabados</dt>
              <dd className="whitespace-pre-line">
                {doorModel.finishes && doorModel.finishes.length > 0
                  ? doorModel.finishes.map((f) => f.name).join(", ")
                  : "-"}
              </dd>
            </dl>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <dl className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <dd className="whitespace-pre-line">
                {format(new Date(doorModel.createdAt), "PPP", { locale: es })}
              </dd>
              <dt className="font-semibold">Última actualización:</dt>
              <dd className="whitespace-pre-line">
                {format(new Date(doorModel.updatedAt), "PPP", { locale: es })}
              </dd>
            </dl>
          </div>
        </CardContent>
