...
// Actions
import { saveDoorPrices } from "../actions/door.actions";
// Types
import type {
  DoorHandlersProps,
  DoorHandlersReturn,
...
} from "./types/door.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveDoorPrices({
      doorId: door.id,
      prices: parsedPrices,
    });
...
const DoorHandlers = ({
  ...
  setLoading,
  setOriginalData,
}: DoorHandlersProps): DoorHandlersReturn => {
  return {
    ...
    handleSavePrices: () =>
      savePricesHandler({ data, door, setData, setLoading, setOriginalData }),
  };
};
