...
import type {
  ...
  SubmitHandlerProps,
} from "./types/extras.handlers.types";
...
const navigateHandler = ({ row, router }: NavigateHandlerProps): void => {
  router.push(`/doors/extras/${row.id}`);
};
...
const submitHandlerCreate = async ({
  ...
  try {
    const { extra, error, success } = await createExtra({
      newImages,
      values,
    });
    ...
    console.error(error);
    toast.error("Error al crear el extra. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { extra, error, success } = await updateExtra({
      id: selectedRow.id,
      newImages,
      toDelete,
      values,
    ...
    if (success && extra) {
      setData((prev) =>
        prev.map((item) => (item.id === extra.id ? extra : item)),
      );
      form.reset();
      setOpenDialog(false);
      setSelectedRow(null);
    ...
    console.error(error);
    toast.error("Error al actualizar el extra. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteExtra({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error("Error al eliminar el extra. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleExtras({
    ...
    console.error(error);
    toast.error("Error al eliminar los extras. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};

const ExtrasHandlers = ({
  form,
  newImages,
  router,
  selectedRow,
  ...
  setToDelete,
  toDelete,
}: ExtrasHandlersProps): ExtrasHandlersReturn => {
  return {
    handleCreate: () =>
      createHandler({
        setExistingImages,
