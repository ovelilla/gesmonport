...
} from "./types/extras.hook.utils.types";

function getColumnsConfig({
  handleDelete,
  handleEdit,
  handleNavigate,
}: GetColumnsConfigProps<Extra>): GetColumnsConfigReturn<Extra> {
  return [
    {
    ...
    {
      cell: ({ row }) => (
        <Link href={`/doors/extras/${row.original.id}`} prefetch={false}>
    ...
    },
    {
      cell: ({ row }) => (
        <RowActions
          ...
          actions={[
            { icon: Eye, label: "Ver extra", onClick: handleNavigate },
