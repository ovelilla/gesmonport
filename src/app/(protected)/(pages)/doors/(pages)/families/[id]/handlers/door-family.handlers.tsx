...
// Actions
import { saveDoorFamilyPrices } from "../actions/door-family.actions";
// Types
import type {
...
} from "./types/door-family.handlers.types";
// Utils
...
const savePricesHandler = async ({
  ...
  try {
    const { prices, error, success } = await saveDoorFamilyPrices({
      doorFamilyId: doorFamily.id,
      prices: parsedPrices,
    });
...
const DoorFamilyHandlers = ({
  data,
  doorFamily,
  setData,
  setLoading,
  setOriginalData,
}: DoorFamilyHandlersProps): DoorFamilyHandlersReturn => {
  return {
