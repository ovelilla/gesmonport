...
} from "./types/doors.hook.utils.types";

function getColumnsConfig({
  handleDelete,
  handleEdit,
  handleNavigate,
}: GetColumnsConfigProps<Door>): GetColumnsConfigReturn<Door> {
  return [
    {
    ...
    {
      cell: ({ row }) => (
        <Link href={`/doors/${row.original.id}`} prefetch={false}>
    ...
    },
    {
      cell: ({ row }) => (
        <RowActions
          ...
          actions={[
            { icon: Eye, label: "Ver puerta", onClick: handleNavigate },
