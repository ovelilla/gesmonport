...
import type {
  ...
  SubmitHandlerProps,
} from "./types/doors.handlers.types";
...
const submitHandlerCreate = async ({
  ...
  try {
    const { door, error, success } = await createDoor({
      newImages,
      values,
    });
    console.log(door);
    ...
    console.error(error);
    toast.error("Error al crear la puerta. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerEdit = async ({
  ...
  try {
    const { door, error, success } = await updateDoor({
      id: selectedRow.id,
      newImages,
      toDelete,
      values,
    ...
    if (success && door) {
      setData((prev) =>
        prev.map((item) => (item.id === door.id ? door : item)),
      );
      form.reset();
      setOpenDialog(false);
      setSelectedRow(null);
    ...
    console.error(error);
    toast.error(
      "Error al actualizar la puerta. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const submitHandlerDelete = async ({
  ...
  try {
    const { success, error } = await deleteDoor({ id: selectedRow.id });
    ...
    console.error(error);
    toast.error("Error al eliminar la puerta. Por favor, inténtalo de nuevo");
  } finally {
    setLoading(false);
  }
};
...
const submitHandlerDeleteMultiple = async ({
  ...
  setLoading(true);
  try {
    const { success, error } = await deleteMultipleDoors({
    ...
    console.error(error);
    toast.error(
      "Error al eliminar las puertas. Por favor, inténtalo de nuevo",
    );
  } finally {
    setLoading(false);
  }
...
const DoorsHandlers = ({
  ...
  setToDelete,
  toDelete,
}: DoorsHandlersProps): DoorsHandlersReturn => {
  return {
    handleCreate: () =>
      createHandler({
        setExistingImages,
